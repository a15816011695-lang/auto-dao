...
| 状态模板 | `${SKILL_DIR}/templates/session-state-template.json` | `session_state.json` 初始化模板 |
...
5. 读取 `${SKILL_DIR}/templates/session-state-template.json`，初始化 `session_state.json`，填入：`topic_id`、`source_path`、`processed_path`、`created_at`、`total_lessons`（来自 roadmap 规划）
6. 跳至 Step 4

**若用户选择继续历史**：
1. **优先读取 `session_state.json`**：获取 `current_lesson`、`current_status`、`counterfactual_mode` 等机器状态（唯一权威源）
...
4. **更新 `session_state.json`（状态唯一权威源）**：
   - `current_lesson` ← 下一课编号
   - `current_status` ← `"lesson_generated"` 或 `"graded"`（视当前阶段）
...
5. **状态一致性自检（强制执行）**：
   ...
   | 2 | `_ai_context.md` 的"当前课时状态"与 `session_state.json` 的 `current_status` 一致 | 读取两个文件对比 |
   ...
   | 5 | 若 `current_status` 为 `"lesson_generated"`，则对应 lesson 文件必须存在 | 检查文件是否存在 |
...
- [x] 用户答案已检查，掌握度：<百分比>
- [x] session_state.json 已更新
- [x] summary.md 已更新
- [x] roadmap_status.md 已更新
- [x] _ai_context.md 已更新
- [x] 🔒 状态自检：{N}/5 项通过
